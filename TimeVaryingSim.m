...
part_max = 3; % max dimension of an agent's input partition
outSize = 1; % agents' output size
numFun = 10; % number of switching functions
uMin = -300; % input lower bound
uMax = 300; % input upper bound
iter = 500; % number of iterations allowed per switching function
gamma = 5e-4; % step size
B = 50;
...
for i = 1:numFun
    ...
    for j = 1:iter
        ...
        update = find(lastUp < counter-B+1);
        if ~isempty(update)
            for k = 1:size(update,1)
                agentUp = update(k);
                ind1 = sum(n(1:agentUp - 1)) + 1;
        ...
        measure = find(lastMeas < counter-B+1);
        if ~isempty(measure)
            for k = 1:size(measure,1)
                agentMeas = measure(k);
                ind3 = agentMeas*outSize - (outSize-1);
        ...
        communicate = find(lastCom < counter-B+1);
        if ~isempty(communicate)
            for k = 1:size(communicate,1)
                agentCom = communicate(k);
                ind1 = sum(n(1:agentCom - 1)) + 1;
        ...
        % true state cost
        trueCost(counter) = (1/2) * uTrue' * R(:,:,i) * uTrue + r(:,i)' * uTrue;
        trueCost(counter) = (1/2) * uTrue'*Q(:,:,i)*uTrue + q(:,i)'*uTrue + (1/2) *yTrue'*P(:,:,i)*yTrue + p(:,i)'*yTrue;
...
lgd = legend('$\alpha \big( k;t_\ell \big)$','$\beta \big( k;t_\ell \big)$');
